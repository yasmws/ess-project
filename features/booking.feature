...
SCENARIO: Realizar com sucesso uma reserva
    Given que o usuário comum de login "ymws" e senha "ILikeBubbleTea" se encontra na página "Lista de Acomodações Disponíveis"
    And o usuário de login "ymws" e senha "ILikeBubbleTea" consegue visualizar a lista de reservas disponíveis e a barra de pesquisa
    When o usuário de login "ymws" escolhe a acomodação "Hotel CIn"
    And o usuário de login "ymws" seleciona "Reservar"
...
SCENARIO: Realizar uma reserva sem estar logado
    ...
    And o usuário comum não logado seleciona a opção "Reservar"
    Then o usuário comum não logado recebe uma mensagem de erro "Você precisa estar logado para reservar uma acomodação"
    And o usuário comum não logado é redirecionado para a página de “Login”

SCENARIO: Visualizar todos os detalhes da acomodação
    Given que o usuário comum de login "ymws" e senha "ILikeBubbleTea" se encontra na página “Lista de Acomodações Disponíveis"
    And o usuário comum de login "ymws" e senha "ILikeBubbleTea" consegue visualizar a lista de reservas disponíveis e uma barra de pesquisa
...
SCENARIO: Adicionar Serviços Adicionais à Reserva
    Given que o usuário comum de login "ymws" e senha "ILikeBubbleTea" está na página "Hotel CIn"
    And o usuário de login "ymws" e senha "ILikeBubbleTea" visualiza informações detalhadas sobre a acomodação "Hotel CIn"
